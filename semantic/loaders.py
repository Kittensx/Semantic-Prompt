...
from patches.bootstrap import category_meta_db
...
META_DB_PATH = Path(__file__).resolve().parents[1] / "semantic" / "meta" / "category_meta.sqlite" 
@dataclass
class TriggerRec:
    raw: str
    norm: str
...
def normalize_pack_keys(data):
    ...
    for k, v in data.items():
        ...
        if nk in normalized:
            print(f"[SemanticPrompt] duplicate normalized key: {k} -> {nk}")
...
class SemanticRegistry:
    ...
    def load_packs(self):
        for file in self.iter_pack_files():
            ...
            if cat_id:
                try:
                    category = category_meta_db.get_category(cat_id)
                except Exception as e:
                    print(f"[SemanticPrompt] WARNING: failed to resolve cat_id '{cat_id}' from DB for {file}: {e}")
            ...
            if not category:
                print(f"[SemanticPrompt] WARNING: skipping pack with no category/cat_id: {file}")
                continue

            category = _norm_token(category)

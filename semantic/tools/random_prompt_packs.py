...
def scan_registry_packs() -> Dict[str, List[str]]:
    if registry is None:
        return {}

    packs: Dict[str, List[str]] = {}

    for cat, entries in registry.packs.items():
        keys = []
        ...
        if keys:
            packs[norm(cat)] = sorted(set(keys))

    return packs
def generate_random_directives(
    *,
    total: int = 6,
    prints: int = 1,
    ...
    if use_registry:
        ...
        packs = scan_packs(Path(packs_dir))

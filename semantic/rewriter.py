...
def _apply_pack_entry(category: str, key: str, tags_out: Dict[str, List[str]], neg_out: List[str], debug: Dict, settings: RewriteSettings, _stack=None, ):
    if _stack is None:
        _stack = set()
    ...
    if isinstance(requires, list):
        ...
        for req in req_items:
            # Allow "cat:key" format, else same category
            if ":" in req:
                req_cat, req_key = req.split(":", 1)
                req_cat = req_cat.strip().lower()
                req_key = req_key.strip().lower()
                if req_cat and req_key:
                    _apply_pack_entry(req_cat, req_key, tags_out, neg_out, debug, settings=settings, _stack=_stack)
            else:
                _apply_pack_entry(category, req.lower(), tags_out, neg_out, debug, settings=settings,  _stack=_stack)
    ...
    for cat, keys in directives.items():
        ...
        for raw in keys:
            ...
            if registry.get_pack(cat, key_lc):
                _apply_pack_entry(cat, key_lc, tags_by_cat, neg_additions, debug, settings=settings)
                ...
                for rcat, rkey in resolved:
                    if rcat == "__literal__":
                        ...
                        _apply_pack_entry(rcat, rkey, tags_by_cat, neg_additions, debug, settings=settings)
                    ...
                    if isinstance(add_list, list):
                        for k in add_list:
                            if isinstance(k, str) and k.strip():
                                _apply_pack_entry(cat, k.strip(), tags_by_cat, neg_additions, debug, settings=settings)

        # Apply best picks after adds
        for cat, (pr, key) in best_pick.items():
            _apply_pack_entry(cat, key, tags_by_cat, neg_additions, debug, settings=settings)
